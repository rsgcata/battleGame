...
foreach ($rounds as $round) {
    ...
    if ($round->wasHeroAttacker()) {
        echo 'The Hero prepares to attack the Monster, dealing ' . $round->getInitialDamageValue()
            . ' damage!' . PHP_EOL;
        ...
        if ($round->getSkillUsedByHero() !== null) {
            echo 'The Hero used the skill ' . $round->getSkillUsedByHero()->getSkillName()
                . '.' . ' His damage increased to ' . $round->getDamageAfterSkill()
                . ' !' . PHP_EOL;
        }
        ...
        if ($round->wasDefenderLucky()) {
            echo 'The Monster got lucky and blocked all the damage dealt by the Hero!' . PHP_EOL;
        } else {
            echo 'The Monster did not get lucky and took a full hit!' . PHP_EOL;
        }

        usleep(700000);

        echo 'The Monster now has ' . $round->getDefenderHealth() . ' health left (took ' . $round->getFinalDamageValue() . ' damage).';
    } else {
        echo 'The Monster prepares to attack the Hero, dealing ' . $round->getInitialDamageValue() . ' damage!' . PHP_EOL;

        usleep(700000);

        if ($round->getSkillUsedByHero() !== null) {
            echo 'The Hero used the skill ' . $round->getSkillUsedByHero()->getSkillName() . '.' . ' Monster\'s damage was decreased to ' . $round->getDamageAfterSkill() . ' !' . PHP_EOL;
        }
        ...
        if ($round->wasDefenderLucky()) {
            ...
            echo 'The Hero did not get lucky and took a full hit!' . PHP_EOL;
        }

        usleep(700000);

        echo 'The Hero now has ' . $round->getDefenderHealth() . ' health left (took ' . $round->getFinalDamageValue() . ' damage).';
    }
